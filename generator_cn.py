...
def main():
    ...
    while True:
        ...
        try:
            count = int(input("要生成的账号数量 (1-50, 默认5): ") or 5)
            count = max(1, min(count, 50))
            username_length = int(input("账号长度 (5-12, 默认7): ") or 7)
            username_length = max(5, min(username_length, 12))
            password_length = int(input("密码长度 (7-17, 默认10): ") or 10)
            ...
            choice = input("输入数字 (1/2/3): ").strip() or "3"
            email_type = {'1':'outlook','2':'hotmail','3':'both'}.get(choice, 'both')

            output_file = input("输出文件名 (默认 accounts.txt): ") or 'accounts.txt'
